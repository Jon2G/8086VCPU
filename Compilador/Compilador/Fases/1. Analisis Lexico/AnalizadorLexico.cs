    ...
    public class AnalizadorLexico : Analizador
    {
        ...
        public override void Analizar()
        {
            ...
            foreach (DocumentLine linea in this.Documento.Lines)
            {
                ...
                //Remover comentarios
                texto = ExpresionesRegulares.Comentarios.Replace(texto, "").Trim();
                ...
                string[] palabras =
                    ExpresionesRegulares.Documento
                        .Split(texto)
                        .Select(s => s.Trim())
                ...
                {
                    token = Token.Identificar(palabra, linea);
                    if (token != null)
                    {
                        if (token.EsValido)
                        {
        ...
        public override void Analizar(List<Tuple<string, DocumentLine>> Lineas)
        {
            ...
            foreach (Tuple<string, DocumentLine> linea in Lineas)
            {
                ...
                foreach (string palabra in palabras)
                {
                    token = Token.Identificar(palabra, linea.Item2);
                    if (token != null)
                    {
                        if (token.EsValido)
                        {
